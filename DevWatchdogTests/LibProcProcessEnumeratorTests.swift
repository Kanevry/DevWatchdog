...
final class LibProcProcessEnumeratorTests: XCTestCase {
    ...
    /// `proc_listpids` view that omits the test runner's own PID. Local runs
    /// (even under xcodebuild) return the full list. Probe once per class
    /// and skip all tests if libproc is restricted in this environment.
    override func setUpWithError() throws {
        ...
        if !probe.map(\.id).contains(selfPID) {
            throw XCTSkip(
                ...
                "(expected on GitHub-hosted CI runners). Feature is behind a disabled " +
                "default flag; tests pass on unrestricted hosts."
            )
        }
    }
